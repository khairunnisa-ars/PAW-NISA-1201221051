...
@section('content')
    ...
    <form action="{{ route('users.store') }}" method="POST" style="max-width: 500px; margin: 0 auto; padding: 20px; border-radius: 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.1); background-color: #fff;">
        ...
        <div style="margin-bottom: 15px;">
            ...
            <input type="text" name="phone" value="{{ old('phone') }}" style="width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 5px; font-size: 16px;">
            @error('phone') <div style="color: #f44336; margin-top:6px;">{{ $message }}</div> @enderror
        </div>
